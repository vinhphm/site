...
export async function compressImages(files: string[]) {
  await Promise.all(
    files.map(async (file) => {
      ...
      if (!format) throw new Error(`Could not determine format of ${file}`)
      if (!(width && height))
      ...
      if (percent > -0.1) {
        console.log(
          c.dim(
            `[SKIP] ${bytesToHuman(size)} -> ${bytesToHuman(outSize)} ${(percent * 100).toFixed(1).padStart(5, ' ')}%  ${file}`
          )
        )
      } else {
        await fs.writeFile(file, outBuffer)
        console.log(
          `[COMP] ${bytesToHuman(size)} -> ${bytesToHuman(outSize)} ${c.green(`${(percent * 100).toFixed(1).padStart(5, ' ')}%`)}  ${file}`
        )
      }
    })
  )
