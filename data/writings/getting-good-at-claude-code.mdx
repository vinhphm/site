...
pubDate: 2025-11-16T10:53:11.234+07:00
...
Like many developing tools out there, Claude Code only supported macOS, Linux, and WSL on Windows when it first came out. But that has changed since then, as the newer versions now support Windows "natively" (still need Git for Windows installed for that sweet Git Bash to work, but hey, it's still progress 🚀).

<OEmbed url="https://x.com/vinhev/status/1943879411696079142" />
...
Getting Claude Code installed is easier than ever. You've got a few options depending on your preference:
...
After installation, navigate to your project and run `claude` to start.

### Working behind corporate firewalls

If you're stuck behind a corporate firewall (we've all been there), Claude Code has you covered with proxy and certificate support:
...
# Skip the proxy for specific domains
export NO_PROXY="localhost,192.168.1.1,example.com,.example.com"

# Custom CA certificates (for those lovely self-signed certs)
export NODE_EXTRA_CA_CERTS=/path/to/ca-certificates.crt
```

**Pro tip:** Don't want to set these environment variables every time? Throw them in your `settings.json` file instead:

- **macOS/Linux**: `~/.claude/settings.json`
- **Windows**: `%USERPROFILE%\.claude\settings.json`

Check out the [official settings documentation](https://code.claude.com/docs/en/settings#available-settings) for the complete list of what you can configure.

If you're dealing with mTLS or more complex auth like NTLM or Kerberos, you might want to look into using an LLM Gateway service that handles that stuff for you.

### IDE integration

You can grab the Claude Code extension for VS Code, Cursor, or Windsurf. It's basically a fancy launcher, but it makes opening Claude Code super smooth and lets you run multiple instances at once. The VS Code extension even respects your font settings and has some nice config options for permissions and .gitignore stuff.
...
### Master the CLAUDE.md file and Claude Skills
...
Claude also supports **Claude Skills**, which are basically reusable knowledge and capabilities you can share across projects and teams.
...
Claude can actually spin up multiple versions of itself to tackle different parts of a problem at the same time. Some cool sub-agents you'll run into:

- **Plan mode**: Claude automatically switches to this when you ask for complex multi-step tasks. It'll think through the strategy before diving into execution
- **Explore**: This one's great—it'll zip through your codebase quickly (runs on Haiku)
- **Dynamic model selection**: Claude's smart enough to pick different models for different tasks

When you see `Task(...)` pop up in the output, that's the sub-agents doing their thing.
...
Claude Code's plugin system is pretty slick now. You can extend it with custom commands, agents, hooks, and MCP servers:

- Grab plugins from marketplaces with `/plugin install`
- Drop custom slash commands in the `.claude/commands/` folder
- Share your plugins with the team through git
- Make sure everything works with `/plugin validate`

And yeah, use `$ARGUMENTS` in your custom commands when you need to pass stuff in.
...
- **Resume old conversations**: Use `--resume` flag to list prior threads (now with branch filtering and search!)
- **Edit in your editor**: Hit <kbd>Ctrl</kbd> + <kbd>G</kbd> to pop your prompt into your favorite text editor
- **Tab completion**: Tab works in bash mode for shell commands, just like you'd expect
...
MCPs (Model Context Protocol) work really well with the plugin system now. Just @mention them or use `/mcp` to toggle them on/off. Some popular ones:

- **Context7**: Pulls up documentation for whatever tech stack you're using
- **Puppeteer**: Great for automated screenshots and testing
- **Project-specific MCPs**: Browse the plugin marketplaces to find what you need

If things aren't working, try `--mcp-debug` for troubleshooting, or run `/doctor` for a full health check.
