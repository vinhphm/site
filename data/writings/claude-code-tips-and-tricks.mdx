...
Claude Code now offers multiple installation methods. The recommended approach uses the official install scripts:
...
If you're working in an enterprise environment with network restrictions, Claude Code supports various configurations:
...
**mTLS authentication:**
For mutual TLS authentication, configure your client certificates through environment variables or configuration files as needed.

**Alternative configuration:**
All environment variables shown above can also be configured in your `settings.json` file instead of using environment variables. This provides a more permanent configuration option that doesn't require setting environment variables each time.

The settings file is located at:
- **macOS/Linux**: `~/.claude/settings.json`
- **Windows**: `%USERPROFILE%\.claude\settings.json`

For a complete list of available settings, check the [official settings documentation](https://code.claude.com/docs/en/settings#available-settings).

**Note:** For advanced authentication methods (NTLM, Kerberos), consider using an LLM Gateway service that supports your authentication method.

### IDE integration

You can install the Claude Code extension for VS Code, Cursor, or Windsurf. While it's essentially just a launcher, it makes opening Claude Code seamless and allows you to run multiple instances in parallel. The VS Code extension also respects editor font settings and includes configuration options for permission modes and .gitignore handling.
...
Claude also supports **Claude Skills** - reusable knowledge and capabilities that can be shared across projects and teams.
...
Claude can spawn multiple instances of itself to work on different parts of a problem simultaneously. Notable sub-agents include:

- **Plan mode**: For strategic planning (enabled with `/plan`)
- **Explore**: Efficiently searches through your codebase (powered by Haiku)
- **Dynamic model selection**: Claude can choose different models for different sub-agents

You'll see something like `Task(...)` in the output when sub-agents are working.
...
Claude Code now has a comprehensive plugin system that extends functionality with custom commands, agents, hooks, and MCP servers. You can:

- Install plugins from marketplaces: `/plugin install`
- Create custom slash commands in `.claude/commands/` folder
- Share plugins with your team via git
- Validate plugins with `/plugin validate`

Use `$ARGUMENTS` in your custom commands to pass parameters.
...
- **Resume old conversations**: Use `--resume` flag to list prior threads, with branch filtering and search
- **Edit in your editor**: Press <kbd>Ctrl</kbd> + <kbd>G</kbd> to edit your prompt in your system's configured text editor
- **Tab completion**: Use tab completion for shell commands in bash mode
...
MCPs (Model Context Protocol) integrate seamlessly through the plugin system. You can enable/disable MCP servers by @mentioning them or through `/mcp`. Popular integrations include:

- **Context7**: Grabs documentation for any tech stack on demand
- **Puppeteer**: For automated screenshots and testing
- **Various project-specific MCPs**: Check plugin marketplaces for your specific needs

Use `--mcp-debug` if you run into configuration issues, or `/doctor` for comprehensive diagnostics.
